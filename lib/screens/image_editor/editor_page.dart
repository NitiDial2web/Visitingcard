...
//From path_provider package
  ...
  Future<void> _createPDF() async {
    ...
    page.graphics
        .drawImage(PdfBitmap(await _readImageData('gmail.png')), Rect.zero);
  ...
  Future<void> saveAndlaunchPdf(List<int> bytes) async {
    ...
    final file =
        File('${directory}/${DateTime.now().millisecondsSinceEpoch}.pdf');
    ...
    for (int i = 0; i <= maxProgress; i++) {
      await Future<void>.delayed(const Duration(seconds: 1), () async {
        ...
        await flutterLocalNotificationsPlugin.show(
            0, 'Visiting Card', 'File Download', platformChannelSpecifics,
            payload: 'item x');
      });
    }
  }
                ...
                child: Container(
                  ...
                  child: InAppWebView(
                    initialUrlRequest: URLRequest(
                      ...
                      // Uri.parse('https://www.w3schools.com/tags/tryit.asp?filename=tryhtml5_a_download')
                    ...
                    onDownloadStartRequest: (controller, url) async {
                      ...
                      // print('Permission.storage.status:${await Permission.storage.status}');
                      print(
                          'Permission.storage.status:${await isStoragePermission}');
                      // await checkPermission();
                      // print(await checkPermission());

                      print("onDownloadStart ${url.url}");
                      print('url:${url.url.path}');
                      // await WebImageDownloader.downloadImageFromWeb('${url.url}');
                      if(androidInfo.version.sdkInt >= 33 ? (await Permission.videos.status.isGranted && await Permission.photos.status.isGranted) : await Permission.storage.status.isGranted) {
                        if (isStoragePermission &&
                            ...
                            isPhotosPermission) {
                          print('if true');
                          String base64Image = url.url.path
                              .split(",")
                              .last;
